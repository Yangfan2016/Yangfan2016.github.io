...
选用 coveralls 可以根据 travis 代码测试后生成的代码覆盖率生成 badge（Github 好多项目都有的）![Build Status](https://travis-ci.org/Yangfan2016/react-yan-progress.svg?branch=master)
...
- 包测试  
	...
	在你的项目根目录下，打开终端运行如下命令，建立链接
	```bash
	$ yarn link
	```
	在你要测试的 demo 项目根目录下，执行如下，然后你就可以这样使用了 `import YanProgress from 'react-yan-progress'`
	```bash
...
👏 欢迎大家一起和我搞 ji（[Github](https://github.com/Yangfan2016)）😊 
